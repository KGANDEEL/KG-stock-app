...
# ==========================================
# 3. القائمة الموسعة للأسهم (تشمل جبل عمر وباتك وكل الإسمنتات)
# ==========================================
saudi_market = {
    # الأسهم المطلوبة حديثاً
    "4250": "جبل عمر", 
    "4110": "باتك",
    # قطاع الإسمنت بالكامل
    ...
    # باقة من الشركات القيادية السابقة لضمان شمولية المنصة
    ...
    "4004": "دله الصحية", "4013": "سليمان الحبيب"
}
...
with tab2:
    ...
    if st.button("تشغيل الفحص الذكي الفوري 🚀"):
        ...
        for index, (code, name) in enumerate(saudi_market.items()):
            status_text.text(f"🔄 جاري معالجة وفحص: {name} ({code})...")
            ...
            time.sleep(0.02)  # تسريع العملية قليلاً
        ...
        if results:
            ...
            # عرض النتائج المصفاة
            st.success(f"✅ تم الفحص! تم العثور على {len(res_df)} شركة تطابق خيار التصفية الخاص بك.")
            if not res_df.empty:
