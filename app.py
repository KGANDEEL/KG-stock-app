...
# قائمة موسعة جداً - تغطي أغلب السوق (يمكنك إضافة أي سهم لاحقاً بنفس الطريقة)
@st.cache_data
def get_full_saudi_market():
    # جمعت لك معظم الشركات الأساسية في السوق السعودي
    return {
...
st.title("🛡️ رادار القناص: المسح الشامل (النسخة الموسعة)")
...
with tab2:
    ...
    if st.button(f"ابدأ المسح (فحص {len(stocks)} شركة) 🚀"):
        with st.spinner("جاري المسح..."):
            with ThreadPoolExecutor(max_workers=30) as executor: # زيادة القوة لـ 30 معالج
